...
# File name: vtidss_ml_aldo_cus_cluster_kmc_rl_0.04.5.py
# Previous: 0.04.4
# VTI Library: vtidss_commit_bq_0.4.py
# MODIFIED NOTE: 
#           -  modified output file for convenient upload to BigQuerry by vtidss_commit_bq_0.2.py
#           -  ONLY upload file gen to OUT-DIR
...
#               -h --help       run for help
#               -u --commitonly only commit file
#
# Command: python vtidss_ml_aldo_cus_cluster_kmc_rl_0.04.4.py -h
#
# Example: python vtidss_ml_aldo_cus_cluster_kmc_rl_0.04.4.py -k 10 -i Aldo.tableA [-o outdir_path] [-limit 1000][ --querry] [--commit]
#
#    Flow:
#    [0.04.3] get data from bigquery
#    [0.04.3] data preprocessing (clean data, data wrangling) 
#    [0.04.3] run K-mean clustering
#    [0.04.3] analyse customer insight
#    [0.04.3] Labeling for each customer
#                  Three type of Labels: Dom, 2-Dom, Mix 
#                  Current rule: gt 55% and twice gt the next
#    [0.04.3] Save data to file
#    [0.04.3] commit to sandbox
#    [0.04.4] commit by INSERT INTO
#    [0.04.4] if corruption, redo INSERT INTO
#    [0.04.4] ONLINE VERIFY by download the uploaded table and check line by line
#    [0.04.4] Solved case of FLOAT64 when upload will truncate data to .6f
#    [0.04.4] vtidss_commit_bq_0.4.py (MUST)
#    [0.04.5] add --commitonly
#
#    Run Command: python vtidss_ml_aldo_cus_cluster_kmc_rl_0.04.4.py -k 10 --query --commit
#
...
try:
  opts, args = getopt.getopt(sys.argv[1:], 'hk:o:d:i:l:n:c:q:u', ['help','kcluster=','outdir=', 'debug','input=','limit=','nopivot','commit','query','commitonly'])
...
if not commitonly:
  ...
  if query:
    ...
    df_bq.to_csv(bq_datafile)
  else:
    print ("[INFO] Read input data from offline file, need update please run again with -q to query new data from Big Query")
    df_bq = pd.read_csv(bq_datafile)
  
  #short data for test
  #df1=df_bq[df_bq['rfm_group']=='Group 1'][:200]
  #df2=df_bq[df_bq['rfm_group']=='Group 2'][:200]
  #df3=df_bq[df_bq['rfm_group']=='Group 3'][:200]
  #df4=df_bq[df_bq['rfm_group']=='Group 4'][:200]
...
if not commitonly:
  ...
  print ("[CLUSTER RESULT] Please check at \n{}\n{}\n{}".format(output_path+"/"+version+"_aldo_cus_cluster_by_product.csv",output_path+"/"+version+"_aldo_cus_cluster_by_product_short.csv",output_path+"/"+version+"_aldo_group_cluster_info.csv"))
...
if (commit or commitonly):
  ...
  commit_filelist = [output_path+"/"+version+"_aldo_cus_cluster_by_product_short.csv",output_path+"/"+version+"_aldo_group_cluster_info.csv",output_path+"/"+version+"_aldo_group_cluster_summary.csv"]
  commit_sandbox_name = ["Aldo.kmc_cus_cluster_rs_of_"+input.split('.')[-1],"Aldo.kmc_cus_cluster_percent_of_"+input.split('.')[-1],"Aldo.kmc_cus_cluster_summary_of_"+input.split('.')[-1]]
  #commit_filelist = [output_path+"/"+version+"_aldo_group_cluster_info.csv",output_path+"/"+version+"_aldo_group_cluster_summary.csv"]
  #commit_sandbox_name = ["Aldo.kmc_cus_cluster_percent_of_"+input.split('.')[-1],"Aldo.ml_cus_cluster_summary_of_"+input.split('.')[-1]]
 
  for i in range(len(commit_filelist)):
    os.system('python vtidss_commit_bq_0.4.py -s {} -d {} --numrow 500 --pause 50 --verify'.format(commit_filelist[i],commit_sandbox_name[i]))
