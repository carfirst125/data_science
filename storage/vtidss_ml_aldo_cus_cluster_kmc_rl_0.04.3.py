...
# File name: vtidss_ml_aldo_cus_cluster_kmc_rl_0.04.3.py
# Previous: 0.04.2
...
#               -h --help       run for help
#
# Command: python vtidss_ml_aldo_cus_cluster_kmc_rl_0.04.3.py -h
#
# Example: python vtidss_ml_aldo_cus_cluster_kmc_rl_0.04.3.py -k 10 -i Aldo.tableA [-o outdir_path] [-limit 1000][ --querry] [--commit]
#
#    Flow:
...
try:
  opts, args = getopt.getopt(sys.argv[1:], 'hk:o:d:i:l:n:c:q', ['help','kcluster=','outdir=', 'debug','input=','limit=','nopivot','commit','query'])
...
if commit:
  ...
  commit_sandbox_name = ["Aldo.ml_cus_cluster_"+input.split('.')[-1],"Aldo.ml_cus_cluster_stat_"+input.split('.')[-1],"Aldo.ml_cus_cluster_summary_"+input.split('.')[-1]]
  for i in range(len(commit_filelist)):
    os.system('python vtidss_commit_bq_0.2.py -s {} -d {} -p 500'.format(commit_filelist[i],commit_sandbox_name[i]))
